...
INSERT INTO seats (row_no, number, room_id)
...
SELECT COUNT(last_name) FROM customers WHERE last_name='Smith';
SELECT COUNT(first_name) FROM customers WHERE first_name IS NULL;
...
SELECT COUNT(*) FROM screenings s JOIN films f ON s.film_id = f.id 
...
-- we have printed each customer id booking screening id once
-- any column that we are selecting must appearing in group by clause
...
-- Having clause
-- whenever we are grouping data while selecting, then we will have to use having clause instead of  Where clause
SELECT * FROM bookings;
SELECT customer_id,screening_id,COUNT(id) FROM bookings 
GROUP BY customer_id,screening_id
...
FROM bookings b JOIN reserved_seat rs
...
-- non corelated nested subquery
SELECT id, start_time FROM screenings
WHERE film_id IN 
	(SELECT id FROM films
    WHERE length_min>120);
-- in these the inner nested query can work all alone also
SELECT first_name,last_name, email FROM customers
WHERE id IN
(SELECT customer_id FROM bookings
WHERE screening_id =1);

-- If we alone run inner query then also, it will running without error as it is non - correlated subquery
SELECT customer_id FROM bookings
WHERE screening_id =1;

 -- Same type with from instead of IN 
 SELECT MAX(no_of_seats) FROM 
 (SELECT booking_id, COUNT(seat_id) AS no_of_seats FROM reserved_seat
 GROUP BY booking_id) b;
 
 -- when inner query can't run independently then it is called correlated subquery
 ...
 FROM (SELECT film_id,COUNT(id) AS id FROM screenings GROUP BY film_id) a;
 
 SELECT name,(SELECT COUNT(id) FROM screenings WHERE film_id = f.id)
 FROM films f;
 
 select concat(first_name,' ',last_name) as full_name from customers;
 
 select substring("Example",3,3) as substring;
-- it has a syntax: substring(string,starting_from,length)
 
 select substring(name,1,3) as shortname from films;
 select substring(name,5,4) as shortname from films;
 select substring(name,-2,2) as shortname from films;
 -- this will return last two letter of all strings
 select substring(name,-6,5) as shortname from films;
 -- negative 6 means 6th letter from the end se beginning hogi
 
 select upper(name) as up_name from rooms;
 select lower(name) as low_name from rooms;
 
 -- Exercise 1
 select concat(name,' ',length_min) as ans1 from films;
 select substring(email,5) as ans2 from customers;
 select lower(first_name),upper(last_name) as ans3 from customers where last_name = 'Smith';
 select substring(name,-3,3) as ans4 from films;
 select concat(substring(first_name,1,3),' ',last_name) as ans5 from customers;
 ...
 -- Exercise 2
 select id,start_time from screenings where date(start_time) = '2017-10-20';
...
-- All bookings that jigsaw got 
select count(*) as Number_of_bookings from bookings where screening_id in 
...
-- 5 Names of customers who made most bookings in October 2017
select concat(c.first_name,' ',c.last_name) as Customer_Name, count(b.id) as No_of_Bookings
...
-- Which Film was shown most in Chaplin room the most often in Oct 2017
select f.name as Name_of_Film,count(film_id) as Number_of_times_Shown
from films f join screenings s on f.id =s.film_id
join rooms r on s.room_id = r.id
where r.id =1
group by film_id
order by Number_of_times_Shown desc
limit 1;

-- 	Customers who made a booking in October 2017
select count(distinct(customer_id)) from bookings;
